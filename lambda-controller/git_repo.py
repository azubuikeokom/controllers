...
class Repo:
    ...
    def clone(self,branch:str="master") -> None:
        ...
        try:
            if os.path.exists(os.path.join(LOCAL_REPO_DIR,local_repo)):
                print(f"path:{os.path.join(LOCAL_REPO_DIR,local_repo)} already exists")
                sys.exit()
            else:
                self.cloned_repo = git.Repo.clone_from(remote_url,os.path.join(LOCAL_REPO_DIR,local_repo),branch=branch)
    ...
    def updateProjectVariables(self,project_name:str,image_uri:str) -> None:
        ...
        try:
            project_dir = os.path.join(LOCAL_REPO_DIR,project_name)
    ...
    def modifyProjectName(self,project_name):
        tokenized_project_name = project_name.split("-")
        del tokenized_project_name[0]
        project_name = "-".join(tokenized_project_name)
        return project_name
